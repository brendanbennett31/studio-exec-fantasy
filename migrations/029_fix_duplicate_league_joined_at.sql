-- duplicate_league()'s bulk `insert into league_members (...) select ...`
-- never set joined_at explicitly, so every copied row fell back to the
-- column default (now()) -- and Postgres freezes now() for the whole
-- statement, so every row in the copy got the exact same timestamp.
-- `order by joined_at` (e.g. the Add Picks team dropdown) has no reliable
-- tiebreak across ties between separate query executions, so a duplicated
-- league's team order could shuffle between page loads.
--
-- Fix: derive each copied row's joined_at from the source league's own
-- joined_at order, offset by whole milliseconds -- preserves the original
-- join order (whoever joined the source league first is still "first" in
-- the duplicate) while guaranteeing every row gets a distinct, strictly
-- increasing value.
