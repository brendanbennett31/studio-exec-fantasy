-- Fixes another bug in duplicate_league(): it used gen_random_bytes(), which
-- comes from the pgcrypto extension. Supabase installs pgcrypto into an
-- `extensions` schema by default, not `public` -- and this function
-- deliberately sets `search_path = public` (same practice as
-- is_league_admin/is_league_member), so it couldn't find it:
--   ERROR: function gen_random_bytes(integer) does not exist
--
-- Switched to gen_random_uuid(), a core Postgres 13+ builtin (always in
-- pg_catalog, no extension dependency at all) -- strip the dashes and take
-- 18 characters, matching the length of the share_token dashboard.html
-- generates client-side at normal league creation.
--
-- create or replace is idempotent -- safe to run even though 018/019 already
-- ran.
